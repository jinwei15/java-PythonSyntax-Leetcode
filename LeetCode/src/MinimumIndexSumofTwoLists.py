...
#         return resultList
        ...
        for i in range(len(list1)):
            reList[list1[i]] = i
        for i in range(len(list2)):
            if list2[i] in reList and i + reList[list2[i]] < indexMin: # and i+reList.get(list2[i]) < indexMin:
                resultList.clear()
                resultList.append(list2[i])
                indexMin = i + reList[list2[i]]
            elif list2[i] in reList and i + reList[list2[i]] == indexMin:
                resultList.append(list2[i])
        return resultList
                # resultList.append(list2[i])
